...
export cmd_CLANG_FORMAT="clang-format-6.0"
...
checks() {
  ...
  if ! [ -x "$(command -v ${cmd_CLANG_FORMAT})" ]; then
    echo "***"
  ...
  CLANG_FORMAT_VERSION="$(${cmd_CLANG_FORMAT} --version)"
...
format_file()
{
  ...
  "${cmd_CLANG_FORMAT}" "${arg}" "${file}" > "${tmpfile}"
